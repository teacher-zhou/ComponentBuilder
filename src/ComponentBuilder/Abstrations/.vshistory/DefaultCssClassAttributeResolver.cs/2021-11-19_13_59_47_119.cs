...
namespace ComponentBuilder.Abstrations
{
    public class DefaultCssClassAttributeResolver : ICssClassResolver, IDisposable
    {
        ...
        {
            if (component.TryGetAttribute<CssClassAttribute>(out var classCssAttribute))
            {
                _cssClassBuilder.Append(classCssAttribute.Css);
            }

            foreach (var parameters in component.GetProperties().Where(m => m.IsDefined(typeof(CssClassAttribute))))
            {
